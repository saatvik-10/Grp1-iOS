...
extension CreatePostViewController {
    ...
    private func setupKeyboardDismissal() {
        ...
        tap.delegate = self // enables simultaneous recognition
    ...
    @IBAction func didTapSaveDraft(_ sender: UIButton) {
        ...
        guard let token = UserDefaults.standard.string(forKey: "authToken") else {
            let alert = UIAlertController(
                title: "Not Logged In",
                message: "Please log in to save a draft.",
        ...
        if let existingDraft = apiDraft {
            updateExistingDraft(existingDraft, token: token, payload: payload)
        } else {
            saveNewDraft(token: token, payload: payload)
        }
    }

    private func updateExistingDraft(_ draft: APIThreadDraft, token: String, payload: APICreateDraftRequest) {
    ...
    private func saveNewDraft(token: String, payload: APICreateDraftRequest) {
    ...
    private func showDraftImage(_ image: UIImage) {
        postImageView.image = image
        addImageButton.isHidden = true
        postImageView.isHidden = false
        imageContainerHeight.constant = 220
