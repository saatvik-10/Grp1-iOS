...
extension PuzzleGenerator {
    ...
    static func runSelfSanityChecks() -> Bool {
        ...
        for _ in 0..<iterations {
            ...
            for (i, _) in engineData.enumerated() {
                let rank = i + 1
                let retPct = PuzzleEngine.getReturnPercent(forRank: rank)
                let companyId = "c\(i+1)"
                let comp = Company(id: companyId, name: "Test Company \(i+1)", description: "Test Description")
            ...
            for idx in 0..<(sortedResults.count - 1) {
                if sortedResults[idx].returnPercent < sortedResults[idx + 1].returnPercent {
        ...
        print("✅ [PuzzleGenerator Validation] Self-sanity checks completed successfully! All randomness and independent ranking assertions passed.")
        return true
    }
}
