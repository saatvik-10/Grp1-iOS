...
class EndingViewController: UIViewController {
    ...
    // MARK: - Legacy IBOutlets (Kept for storyboard compatibility)
    ...
    let kahnemanEndingQuotes: [KahnemanQuote] = [
        KahnemanQuote(text: "The goal of investing is not to avoid risk, but to understand it well enough to take the right one.", author: "Daniel Kahneman", endingType: .success),
        KahnemanQuote(text: "Investors often accept a smaller, certain loss today to avoid the emotional pain of a larger, uncertain one tomorrow.", author: "Daniel Kahneman", endingType: .partialFailure),
        KahnemanQuote(text: "Confidence in financial decisions often reflects a good story, not a good understanding of probabilities.", author: "Daniel Kahneman", endingType: .failure),
        KahnemanQuote(text: "What investors see is all there is — past prices anchor expectations, even when the future has already changed.", author: "Daniel Kahneman", endingType: .criticalFailure)
    ]
    ...
    private var isSuccess: Bool {
        return endingType == .success || endingType == .partialFailure
    }
    ...
    override func viewDidLoad() {
        ...
        // Remove storyboard views to replace with programmatic UI
    ...
    private func buildUI() {
        ...
        stack.addArrangedSubview(padded(makePerformanceCard(), h: 16, v: 0))
        stack.addArrangedSubview(sectionLabel("Detected Biases", icon: "brain.head.profile"))
        stack.addArrangedSubview(padded(makeBiasesStack(), h: 16, v: 0))
        stack.addArrangedSubview(sectionLabel("Insights", icon: "quote.opening"))
        stack.addArrangedSubview(padded(makeQuoteCard(), h: 16, v: 0))
    ...
    private func makeBanner() -> UIView {
        ...
        var bgColor = C.charcoal
        var iconColor = C.red
        var iconName = "xmark"
        var titleText = "Systemic Collapse"
